;;;; Created on 2008-09-18 14:59:00
...
(defun algorithm-compile (package kb strm)
  (mlcl-kb:kb-open kb)
  (let ((compinfo (make-compiler-info)))
    (algorithm-compile-header package kb strm)
    (let ((cls-list))
      (dolist (el (mlcl-kb:kb-interned-elements kb))
        (if (and (typep el 'mlcl-kb:cls) 
                 (mlcl-kb:cls-has-supercls el '|dataset|::|DatasetThing|))
            (push el cls-list)))
      (algorithm-compile-clses package cls-list strm compinfo))  
    (algorithm-compile-trailer package strm))
  (mlcl-kb:kb-close kb))
...
(defun algorithm-compile-header (package kb strm)
...
(defun algorithm-compile-trailer (package strm)
...
(defun algorithm-compile-clses (package cls-list strm compinfo)
  (dolist (cls cls-list)
    (algorithm-compile-cls cls strm compinfo))
  (dolist (typ (compiler-info-enum-types compinfo))
    (algorithm-compile-enum typ strm))
...
(defun algorithm-compile-cls (cls strm compinfo)
  ...
  (dolist (slot (mlcl-kb:cls-direct-template-slots cls))
    ...
    (algorithm-compile-slot slot strm compinfo))
  (format strm "))~%")
  (format strm "~%~%"))

(defun algorithm-compile-slot (slot strm compinfo)
  (format strm "~%	(|~A|~%	 "
          (frame->lisp-name slot))
  (algorithm-compile-slot-type slot strm compinfo)
  (format strm "~%	 :accessor ~A)" 
          (format nil "|~A|" (frame->lisp-name slot))))

(defun algorithm-compile-slot-type (slot strm compinfo)
...
(defun algorithm-compile-enum (slot strm)
