...
;  (format t "<= ~A   ~A~%" name seed)
  (cond 
   ...
   ((eq name 'protege-ns:|own_slot_value|)
    (push (make-slot-values% :slot (seed-slot-reference seed) :vals (nreverse (seed-vals seed))) (seed-own-slot-values parent-seed)))
   ((eq name 'protege-ns:|superclass|)
   ...
   ((eq name 'protege-ns:|template_facet_value|)
    (push (make-facet-values% :slot (seed-slot-reference seed) :facet (seed-facet-reference seed) 
...
(defmethod kb-element-export-to-lxml (kb (element instance))
  ...
  (let ((name (list 'protege-ns:|name| (frame-name element)))
        ...
        (ownsv (mapcar #'(lambda (x) (cons 'protege-ns:|own_slot_value| 
                                           (cons 
                                            (list 'protege-ns:|slot_reference| (frame-name (slot-values%-slot x)))
                                            (make-lxml-from-values (slot-values%-vals x)))))
...
(defmethod kb-element-export-to-lxml (kb (element cls))
  ...
  (let ((lxml (call-next-method))
        ...
        (tfv (mapcar #'(lambda (x) (cons 'protege-ns:|template_facet_value|
                                           (cons 
                                            (list 'protege-ns:|slot_reference| (frame-name (facet-values%-slot x)))
                                            (cons 
                                             (list 'protege-ns:|facet_reference| (frame-name (facet-values%-facet x)))
                                             (make-lxml-from-values (facet-values%-vals x))))))
